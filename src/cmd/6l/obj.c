...
/* Use 9root instead of system root */
static char *sysroot;
static char sysrootbuf[1024];

static char*
sysrootpath(char *abs)
{
  int n;

  if(sysroot == nil || sysroot[0] == 0)
	return abs;
  if(abs == nil || abs[0] != '/')
	return abs;

  n = snprint(sysrootbuf, sizeof(sysrootbuf), "%s%s", sysroot, abs);
  if(n <= 0 || n >= sizeof(sysrootbuf))
	return abs;
  return sysrootbuf;
}
...
/* Switch to 9root or fallback to system root */
	char *p;
	p = sysrootpath(library[i]);
	if(p != library[i] && access(p, AREAD) >= 0)
	  objfile(p);
	else
	  objfile(library[i]);
